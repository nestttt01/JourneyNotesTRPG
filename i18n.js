(function () {
    ...
    const M = {
        ...
        '未知': ['Unknown', '不明'],
        '未命名存檔': ['Untitled Save', '無題のセーブ'],
        '保留人物，隨機世界／情境': ['Keep Characters, Randomize Scenario', '人物を残してシナリオ生成'],
        '人物與世界全部隨機': ['Randomize Everything', 'すべてランダム生成'],
        ...
        '暫時無法取得瀏覽器容量資訊；存檔功能仍可正常使用。': ['Storage details are temporarily unavailable; saves still work.', '容量情報を一時的に取得できませんが、セーブ機能は通常どおり使えます。'],
        '目前沒有遊戲存檔；建立存檔後會在這裡提醒備份。': ['No game saves yet. Backup reminders appear here after you create one.', 'ゲームセーブはまだありません。作成後、ここにバックアップ通知が表示されます。'],
        ...
        '請先建立遊戲存檔。': ['Create a game save first.', '先にゲームセーブを作成してください。'],
        ...
        '完成 API 驗證並選好模型後，先建立角色與情境，再進入存檔輸入第一個行動，就可以開始單人 TRPG。': ['After verifying the API key and choosing a model, create characters and scenarios, open a save, and type your first action to start the solo TRPG.', 'API認証とモデル選択が終わったら、キャラクターとシナリオを作成し、セーブを開いて最初の行動を入力するとソロTRPGを開始できます。'],
        '5. 驗證失敗時': ['5. If Verification Fails', '5. 認証に失敗したとき'],
        '請先確認服務商與金鑰來源一致，再檢查金鑰是否完整貼上、是否有多餘空白、帳號是否已開通 API 權限。OpenRouter 模型清單會依帳號與服務狀態變動。': ['First confirm that the provider matches the key source, then check that the key was pasted completely, has no extra spaces, and that the account has API access enabled. OpenRouter model lists can change by account and service status.', 'まずサービス提供元とキーの発行元が一致しているか確認し、キーが完全に貼り付けられているか、余分な空白がないか、アカウントでAPI権限が有効か確認してください。OpenRouterのモデル一覧はアカウントやサービス状態で変わることがあります。'],
        ...
        '先設定玩家與 NPC，再建立世界觀與第一個情境。進入存檔後輸入第一個行動，遊玩幾回合後再回角色面板微調狀態，會比一開始填滿所有欄位更順。': ['Set up the player and NPCs first, then create the world and first scenario. After entering a save, type the first action; after a few turns, return to the character panel to fine-tune states. This is smoother than filling every field at the start.', 'まずプレイヤーとNPCを設定し、世界観と最初のシナリオを作ります。セーブに入ったら最初の行動を入力し、数ターン遊んでからキャラクターパネルで状態を微調整すると、最初から全項目を埋めるより進めやすくなります。'],
