...
/* ======= 系統核心變數 ======= */
        ...
        function getLanguageInstruction(mode) {
            ...
            const common = "無論角色設定如何，都必須遵守本段輸出語言規則。JSON 欄位名稱仍使用英文鍵名。";
            const rules = {
                "zh-tw": "輸出語言：繁體中文。旁白、NPC 台詞、選項、系統訊息一律使用自然繁體中文。可少量使用外語稱呼或專有名詞，但不要讓玩家必須讀外語才能理解。",
                "en": "Output language: English. Narration, NPC dialogue, options, and system-facing text must be in natural English.",
                "ja": "出力言語：日本語。地の文、NPCの台詞、選択肢は自然な日本語で書く。ただし JSON keys remain in English.",
        ...
        function syncLanguageMode(mode) {
            const nextMode = mode || "zh-tw";
            if (currentScenario) currentScenario.languageMode = nextMode;
                ...
                details.innerHTML = `
                    ...
                    <div class="foldable-content">
                        ...
                        <div class="anime-sheet">
                            ...
                            <div><label>說話習慣 / 語氣</label><input type="text" id="npc-speech-${index}" value="${escapeStatusHtml(npc.details?.speech || '')}"></div>
        ...
        function loadPresetToForm(id) {
            ...
            syncLanguageMode(p.languageMode || 'zh-tw');
        ...
        function openStatusModal() {
            ...
            syncLanguageMode(currentScenario.languageMode || 'zh-tw');
        ...
        function getAdventureJournalEntries(saveId = journalSelectedSaveId) {
            ...
            if (!save) return [];
            return splitAdventureLog(save.log).map((text, index) => ({ text, index }));
        }
...
5. 若目標情境是輔助旁白／玩家不在場模式，禁止把操作者輸入當成玩家角色發言；只有明確回歸指令才能讓玩家角色重新登場。`;
        ...
        function renderChatPage(pageIndex) {
            ...
            if (currentLog.length > 0) {
                ...
                const systemMsg = document.createElement('div'); systemMsg.className = 'system-msg'; systemMsg.innerHTML = "<i>— 此情境/支線尚無對話。請輸入動作，或讓 AI 生成開場 —</i>"; msgBox.appendChild(systemMsg); msgBox.scrollTop = msgBox.scrollHeight;
                
                const btn = document.createElement('button'); btn.className = 'opt-btn'; btn.style.borderColor = 'var(--accent-neon)'; btn.style.width = 'fit-content'; btn.style.alignSelf = 'center'; btn.innerHTML = '🎲 讓 AI 根據「情境設定」隨機生成開場事件';
                btn.onclick = () => { 
                    optArea.innerHTML = ''; 
        ...
        function loadGame(id) {
            ...
            const msgBox = document.getElementById('dialogue-box'); 
            const sysMsg = document.createElement('div'); sysMsg.className = 'system-msg'; sysMsg.innerText = "— 遊戲紀錄已載入 —"; msgBox.appendChild(sysMsg); msgBox.scrollTop = msgBox.scrollHeight;
...
只輸出 JSON：{"attribute":"str|dex|con|int|wis|cha","difficulty":"easy|normal|hard|extreme","reason":"30字內理由"}`;
        ...
        async function sendDiceChoice() {
            ...
            document.getElementById('ui-target-typing').innerText = '判定引擎';
            document.getElementById('loading').style.display = 'block';
...
${transitionRule}`;
        ...
        function updatePlayerInputPlaceholder() {
            ...
            if (!input) return;
            if (creatorInputArmed) {
                input.placeholder = '輸入本回合的創作者指令...';
                return;
            }
            input.placeholder = getSceneParticipationMode() === 'narrator'
                ? '輸入輔助旁白，或點「神」下達創作者指令...'
                : '輸入角色行動，或點「神」下達創作者指令...';
        }
        ...
        function setCreatorInputMode(enabled, focusInput = true) {
            ...
            if (button) {
                ...
                button.title = creatorInputArmed ? '關閉持續創作者指令模式' : '開啟持續創作者指令模式';
                ...
                if(!getCurrentGameOver() && parsedData.options && Array.isArray(parsedData.options)) {
                    ...
                    parsedData.options.forEach(opt => {
                        ...
                        if (option.check && DICE_STATS[option.check]) {
                            ...
                            checkLabel.textContent = `${DICE_STATS[option.check].code} 檢定`;
