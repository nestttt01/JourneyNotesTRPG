...
"核心準則：角色設定頁與遊戲頁都有「核心準則」小標籤，寫進去的鐵則 AI 每回合都會優先讀取、不會遺忘也不可違反，且隨配置保存、兩邊同步。除了世界觀（例如「這世界沒有魔法」）與人物鐵律（例如「主角必須一直戴著面具」），它也很適合鎖定當下場景：例如某位 NPC 設定裡存在、但這一場並不在場時，寫上「B 目前不在此情境」就能有效避免 AI 突然讓他登場。": ["Core Rules: both the character page and the game screen have a \"Core Rules\" tab. Iron rules you write there are read first every turn, never forgotten and never broken, and are saved with the preset and synced on both sides. Besides world rules (e.g. \"there is no magic in this world\") and character laws (e.g. \"the hero must always wear a mask\"), it is great for locking down the current scene: if an NPC exists in the setup but is not present this scene, writing \"B is not in this scene right now\" effectively stops the AI from suddenly bringing them in.", "コアルール：キャラ設定ページとゲーム画面の両方に「コアルール」タブがあります。書いた鉄則は毎ターン最優先で読まれ、忘れられず破られず、プリセットと共に保存され両側で同期します。世界観（例：「この世界に魔法はない」）や人物の鉄則（例：「主人公は常に仮面を着ける」）のほか、現在の場面を固定するのにも有効：ある NPC が設定上は存在してもこの場面にいない時、「B は今この場面にいない」と書けば、AI が急に登場させるのを効果的に防げます。"],
...
"玩家、旁白與「神」模式：一般輸入視為玩家角色的行動。輸入 ［切換旁白］／［旁白］ 可改用場外視角安排環境、鏡頭與 NPC，此時玩家角色不預設在場；輸入 ［切換玩家］／［玩家］ 回到玩家角色。「神」按鈕是創作者模式，可直接調整場景、時間、人物或事件，且不會被 NPC 當成玩家說出口的話。想修正已送出文字、複製或收藏片段時，桌機在對話上按右鍵、手機長按即可開選單。": ["Player, Narrator and \"God\" modes: normal input is your character’s action. Type ［切換旁白］/［旁白］ to switch to an off-stage view and arrange scenery, camera and NPCs, with the player not assumed present; type ［切換玩家］/［玩家］ to return. The \"God\" button is creator mode: adjust scene, time, characters or events directly, and NPCs never hear it as the player speaking. To edit sent text, copy or clip a passage, right-click (desktop) or long-press (mobile) a message for a small menu.", "プレイヤー・ナレーター・「神」モード：通常入力はプレイヤーの行動です。［切換旁白］/［旁白］で場外視点に切替え環境・カメラ・NPC を配置でき、プレイヤーは在場扱いになりません。［切換玩家］/［玩家］で戻ります。「神」ボタンは創作者モードで場面・時間・人物・出来事を直接調整でき、NPC には発言として聞こえません。送信済みテキストの修正・コピー・抜粋保存は、PC は右クリック、スマホは長押しで小メニューが開きます。"],
