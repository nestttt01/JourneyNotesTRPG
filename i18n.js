(function () {
    ...
    const M = {
"玩家、旁白與「神」模式：一般輸入視為玩家角色的行動。輸入 ［切換旁白］／［旁白］ 可改用場外視角安排環境、鏡頭與 NPC，此時玩家角色不預設在場；輸入 ［切換玩家］／［玩家］ 回到玩家角色。": ["Player, Narrator and \"God\" modes: normal input is your character’s action. Type ［切換旁白］/［旁白］ to switch to an off-stage view and arrange scenery, camera and NPCs, with the player not assumed present; type ［切換玩家］/［玩家］ to return.", "プレイヤー・ナレーター・「神」モード：通常入力はプレイヤーの行動です。［切換旁白］/［旁白］で場外視点に切替え環境・カメラ・NPC を配置でき、プレイヤーは在場扱いになりません。［切換玩家］/［玩家］で戻ります。"],
"「神」按鈕是創作者模式，可直接調整場景、時間、人物或事件，且不會被 NPC 當成玩家說出口的話。想修正已送出文字、複製或收藏片段時，桌機在對話上按右鍵、手機長按即可開選單。": ["The \"God\" button is creator mode: adjust scene, time, characters or events directly, and NPCs never hear it as the player speaking. To edit sent text, copy or clip a passage, right-click (desktop) or long-press (mobile) a message for a small menu.", "「神」ボタンは創作者モードで場面・時間・人物・出来事を直接調整でき、NPC には発言として聞こえません。送信済みテキストの修正・コピー・抜粋保存は、PC は右クリック、スマホは長押しで小メニューが開きます。"],
"核心準則：角色設定頁與遊戲頁都有「核心準則」小標籤，寫進去的鐵則 AI 每回合都會優先讀取、不會遺忘也不可違反，且隨配置保存、兩邊同步。": ["Core Rules: both the character page and the game screen have a \"Core Rules\" tab. Iron rules you write there are read first every turn, never forgotten and never broken, and are saved with the preset and synced on both sides.", "コアルール：キャラ設定ページとゲーム画面の両方に「コアルール」タブがあります。書いた鉄則は毎ターン最優先で読まれ、忘れられず破られず、プリセットと共に保存され両側で同期します。"],
"除了世界觀（例如「這世界沒有魔法」）與人物鐵律（例如「主角必須一直戴著面具」），它也很適合鎖定當下場景：例如某位 NPC 設定裡存在、但這一場並不在場時，寫上「B 目前不在此情境」就能有效避免 AI 突然讓他登場。": ["Besides world rules (e.g. \"there is no magic in this world\") and character laws (e.g. \"the hero must always wear a mask\"), it is great for locking down the current scene: if an NPC exists in the setup but is not present this scene, writing \"B is not in this scene right now\" effectively stops the AI from suddenly bringing them in.", "世界観（例：「この世界に魔法はない」）や人物の鉄則（例：「主人公は常に仮面を着ける」）のほか、現在の場面を固定するのにも有効：ある NPC が設定上は存在してもこの場面にいない時、「B は今この場面にいない」と書けば、AI が急に登場させるのを効果的に防げます。"],
"角色成長": ["Character Growth", "キャラ成長"],
"達成重大成就可獲得成長點，用來提升屬性或新增擅長領域。": ["Reach major achievements to earn growth points for raising stats or adding areas of expertise.", "重大な実績で成長ポイントを得て、能力値の強化や得意分野の追加に使えます。"],
...
"進入「劇本創角」後，可在角色設定、情境設定與遊戲設定之間切換。角色設定編輯玩家與每位 NPC 的頭像、名稱、人物資料與說話習慣；情境設定建立世界觀、身分、NPC 在場定位與轉場規則；遊戲設定管理配置名稱、輸出語言、難度與備份。情境也可全部交給 AI 隨機生成，結果會先讓你確認才套用。": ["In \"Create Scenario\" you switch between Character, Scenario and Game settings. Character settings edit the avatar, name, profile and speech style of the player and each NPC; Scenario settings define the world, roles, who is present and scene-transition rules; Game settings manage the preset name, output language, difficulty and backups. You can also let the AI randomize scenarios, with results shown for confirmation before they apply.", "「シナリオ作成」ではキャラ設定・シナリオ設定・ゲーム設定を切り替えます。キャラ設定でプレイヤーと各 NPC のアイコン・名前・プロフィール・口調を、シナリオ設定で世界観・立場・登場状況・転場ルールを、ゲーム設定でプリセット名・出力言語・難易度・バックアップを管理。シナリオは AI にランダム生成させることもでき、結果は適用前に確認できます。"],
"本場目標：每個情境可選填「本場目標」。填了之後 DM 會讓劇情自然朝目標推進、鋪陳線索與機會，但不會硬拉你或無視你的選擇；留空則自由發展。目標達成時會觸發章節收束。": ["Scene Goal: each scenario can have an optional \"Scene Goal\". With one set, the GM steers the story toward it and lays out clues and openings, but never railroads you or ignores your choices; leave it blank for free play. Reaching the goal triggers a chapter close.", "本場の目標：各シナリオに任意で「本場の目標」を設定できます。設定すると GM は物語を自然にそこへ導き手掛かりや機会を用意しますが、無理に引っ張ったり選択を無視したりはしません。空欄なら自由展開。達成すると章の締めくくりが発生します。"],
"直接在下方輸入角色想說或想做的事，也可以點選 AI 提供的選項。行動有風險時用骰子按鈕做 D20 判定，系統會依角色屬性、難度與骰值決定成敗，符合角色「擅長領域」時還會加值。選項旁若標了屬性，代表選它就是用那個屬性檢定。": ["Type what your character says or does below, or click one of the AI-offered options. For risky actions use the dice button for a D20 check: success is decided by your stats, the difficulty and the roll, with a bonus when the action matches an \"area of expertise\". If an option shows a stat, choosing it means a check with that stat.", "下の欄にキャラのセリフや行動を打つか、AI の選択肢を選びます。危険な行動はダイスボタンで D20 判定：能力値・難易度・出目で成否が決まり、「得意分野」に合えば加算。選択肢に能力値が表示されていれば、それを選ぶとその能力値で判定します。"],
"HP 與 SAN 代表角色的生命與精神。任一項偏低時會進入危機區間：相關檢定變難，畫面也會出現暗角、顆粒、掃描線、短暫亂碼與狀態訊號，越接近歸零越強烈。配色會跟著目前介面主題走，不是寫死的顏色。": ["HP and SAN are your character’s body and mind. When either runs low you enter the crisis zone: related checks get harder, and the screen shows vignetting, grain, scan lines, brief glitching and status signals that intensify the closer you get to zero. Colors follow your current UI theme, not a fixed palette.", "HP と SAN は肉体と精神です。どちらかが低くなると危機ゾーンに入り、関連判定は難しくなり、画面にはビネット・粒子・走査線・一瞬のノイズ・状態信号が現れ、ゼロに近づくほど激しくなります。色は固定色ではなく現在の UI テーマに従います。"],
"干擾強度可調：狀態面板的「顯示與輸出設定 → 低狀態干擾」可選完整／減弱／關閉。減弱只保留暗角與輕微亂碼、拿掉大字與選項漂移；關閉則完全不進干擾。系統的「減少動態效果」偏好也會被尊重。": ["You can tune the interference: in the status panel under \"Display & Output → Low-state effects\" choose Full / Reduced / Off. Reduced keeps only the vignette and mild glitching, dropping the large text and option drift; Off disables it entirely. Your system \"reduce motion\" preference is also respected.", "演出強度は調整可能：ステータスパネルの「表示と出力設定 → 低ステート演出」で「フル／控えめ／オフ」を選べます。控えめはビネットと軽いノイズのみ残し、大きな文字や選択肢のずれを外します。オフは完全無効。OS の「視差を減らす」設定も尊重されます。"],
"低 SAN 選項污染：SAN 很低時，AI 選項可能變得不可靠——你點的選項有機率瞬間變色、改成一個隱藏的失控行動並直接送出檢定。越低機率越高，但有冷卻、不會連續發生；沒觸發時，點選仍像平常一樣先填入輸入框。": ["Low-SAN option corruption: at very low SAN the AI’s options can turn unreliable — the option you click may instantly change color, become a hidden loss-of-control action and be sent for a check immediately. The chance rises the lower SAN gets, but has a cooldown so it won’t chain; when it doesn’t fire, clicking just fills the input box as usual.", "低 SAN の選択肢汚染：SAN が非常に低いと AI の選択肢が不安定になり、押した選択肢が瞬時に変色して隠れた「制御喪失」行動に変わり、即座に判定へ送られることがあります。SAN が低いほど発生率は上がりますが、連続しないようクールダウンがあり、未発動なら通常どおり入力欄に入るだけです。"],
"三種難度：標準適合日常與長篇，歸零不會 Game Over；困難會提高檢定難度，歸零時擲一次生死檢定看能否保命，NPC 每次死亡只有一次復活機會；極限更難，NPC 死亡無法復活。": ["Three difficulties: Standard suits slice-of-life and long play, and hitting zero is never Game Over; Hard raises the check difficulty and, at zero, rolls one life-or-death check to see if you survive, with only one revival chance per NPC death; Extreme is harder still, and NPC deaths are permanent.", "3つの難易度：標準は日常や長編向きでゼロでもゲームオーバーになりません。困難は判定難度を上げ、ゼロ時に生死判定を1回して生存可否を決め、NPC の死は1回だけ蘇生可。極限はさらに難しく、NPC の死は永久です。"],
"極限模式的最後掙扎：極限下歸零不會直接結束，而是逼你擲一次「生死檢定」。通過就撐住，並獲得接下來幾回合的「恢復期護盾」——這期間再歸零也會被硬撐住不死，讓你有時間穩住狀態。可重複觸發，但每次都可能失敗。": ["Extreme mode’s last stand: in Extreme, hitting zero doesn’t end things outright — you are forced into a \"life-or-death check\". Pass it and you hold on, gaining a \"recovery shield\" for the next few turns during which hitting zero again is absorbed instead of killing you, giving you time to stabilize. It can repeat, but each attempt can still fail.", "極限モードの最後の抵抗：極限ではゼロでも即終了せず、「生死判定」が強制されます。通れば踏みとどまり、次の数ターン「回復シールド」を得て、その間は再びゼロでも死なずに耐えられ、立て直す時間ができます。繰り返し発生しますが毎回失敗の可能性もあります。"],
"喘息：遊戲頁頂列的「喘息」鈕可主動回復——依角色體質與感知擲檢定，回一些 HP／SAN，但有上限（不能補滿）、也不能連按（要先做一次行動）。它是脫離死亡螺旋的救命繩，不是補血手段。": ["Catch your breath: the \"Catch Breath\" button in the game header lets you recover on purpose — it rolls based on your character’s CON and WIS to restore some HP/SAN, but has a cap (never full) and can’t be used twice in a row (act once between uses). It’s a lifeline out of a death spiral, not a full heal.", "ひと息：ゲーム上部の「ひと息」ボタンで自発的に回復——体質と感知で判定し HP／SAN を少し回復しますが、上限があり（満タン不可）連続使用も不可（間に1回行動が必要）。死のスパイラルから抜ける命綱で、全回復手段ではありません。"],
"恢復道具：劇情中獲得的補給（急救包、鎮靜劑等）可能帶有回復效果，會在角色面板道具區出現「使用」鈕；用了由程式回復並消耗該道具。多數道具沒有效果，只是純敘事物品。": ["Recovery items: supplies gained in the story (medkits, sedatives, etc.) may carry a restore effect and show a \"Use\" button in the items area of the character panel; using one restores by the program and consumes it. Most items have no effect and are purely narrative.", "回復アイテム：物語で得た補給品（救急箱・鎮静剤など）は回復効果を持つことがあり、キャラパネルの所持品欄に「使う」ボタンが出ます。使うとプログラムが回復し消費します。多くのアイテムは効果がなく純粋な物語用です。"],
"角色成長（成就導向）：達成真正重大的成就（完成主線大任務、跨過重大難關、劇情轉捩點等；日常瑣事不算）會獲得成長點。在角色面板的「角色成長」區可花點提升某項屬性或新增一條擅長領域；稱號會依累積成就自動升階。": ["Character growth (achievement-based): reaching a genuinely major achievement (finishing a main quest, overcoming a big threat, a story turning point; trivial tasks don’t count) earns a growth point. In the \"Character Growth\" section of the panel you can spend points to raise a stat or add an area of expertise; your title auto-ranks by total achievements.", "キャラ成長（実績ベース）：本当に重大な実績（メインクエスト達成・重大な難関の突破・物語の転機など。些細な用事は対象外）で成長ポイントを得ます。パネルの「キャラ成長」欄でポイントを使い能力値を上げるか得意分野を追加でき、称号は累積実績で自動昇格します。"],
"章節收束：確實達成情境的「本場目標」時，AI 會寫下一段收束敘事並給你成長點，讓一局能漂亮收尾，而不只是無限漂流或墜落。此外，NPC 好感滿值、關係徹底破裂或死亡時，系統會自動在 Flags 留下里程碑標籤，重大節點不會漏記。": ["Chapter closure: when you truly reach a scenario’s \"Scene Goal\", the AI writes a closing beat and grants a growth point, so a run can end well rather than just drifting or dying. Also, when an NPC’s affection maxes out, a relationship breaks completely, or an NPC dies, the system auto-adds a milestone flag so major beats are never missed.", "章の締めくくり：シナリオの「本場の目標」を確かに達成すると、AI が締めの叙述を書き成長ポイントを付与します。ただ漂流・墜落せず綺麗に締められます。また NPC の好感度満タン・関係の完全決裂・NPC の死亡時は、システムが自動でマイルストーンのフラグを残し重要な節目を取りこぼしません。"],
"冒險日誌與日記：角色面板可查看與修改玩家狀態、NPC、道具、Flags、任務、劇情摘要與角色關係，AI 每回合都會參考。冒險日誌記錄完整事件；日記則收藏你想保留的對話——在對話泡泡右鍵（手機長按）選「收藏這段」，再點另一則當結尾即可，還能把整篇存成長圖。": ["Adventure log & diary: the character panel lets you view and edit player status, NPCs, items, flags, tasks, the story summary and relationships, all referenced by the AI each turn. The adventure log records full events; the diary keeps conversations you want to save — right-click (long-press on mobile) a bubble, choose \"Clip this\", then click another to mark the end. You can also export the whole diary as a long image.", "冒険日誌と日記：キャラパネルでプレイヤー状態・NPC・所持品・フラグ・タスク・物語要約・関係を確認/編集でき、AI が毎ターン参照します。冒険日誌は出来事を記録し、日記は残したい会話を保存——吹き出しを右クリック（スマホ長押し）で「この範囲を保存」を選び別の吹き出しを終点に。日記全体を長い画像に書き出すこともできます。"],
"故事拉長後，AI 可能變遲鈍或接不上，這是模型本身的限制。三個改善方向：一、選強一點的模型（長篇／重連貫選標 ▪ 的大廠款；輕量款快省但易忘）。二、壓縮記憶：多用「整理摘要／整理紀錄」，關鍵設定寫進核心準則或角色面板。三、隨時換模型：遊戲中可切換不必重開。若 AI 混亂，開「神」模式直接說明目前場景、在場角色與想聚焦的方向。": ["Over a long story the AI may get dull or lose the thread — a limit of the model. Three fixes: (1) pick a stronger model (for long or coherent play choose the ▪-marked flagship models; light ones are fast and cheap but forget more); (2) compress memory — use \"Organize summary / log\" often and put key settings in Core Rules or the panel; (3) switch models any time mid-game without restarting. If the AI is confused, use \"God\" mode to state the current scene, who is present and what to focus on.", "物語が長引くと AI が鈍くなったり話を見失うことがあります——モデルの限界です。改善は3つ：①強いモデルを選ぶ（長編・一貫性重視は ▪ 付きの大手。軽量版は速く安いが忘れやすい）。②記憶を圧縮：「要約整理／記録整理」を活用し重要設定はコアルールやパネルへ。③いつでもモデル変更可。混乱時は「神」モードで現在の場面・在場者・焦点を伝えます。"],
"每份遊戲紀錄只綁定一份配置：儲存配置只更新目前配置；另存配置只備份配置、不改綁紀錄；另存新檔會複製目前紀錄並建立專屬配置。匯出備份會保存遊戲紀錄、角色配置、情境配置與角色頭像，方便換裝置或恢復進度，但不包含 API Key；匯入後會保留目前裝置原本的介面語言與外觀設定。": ["Each save binds to one preset: saving a preset only updates the current one; \"save as preset\" only backs up the preset without rebinding the save; \"save as new\" copies the current save and creates its own preset. Exporting a backup keeps saves, character presets, scenario presets and avatars for moving devices or restoring progress, but never the API key; after importing, this device keeps its own UI language and appearance.", "各セーブは1つのプリセットに紐づきます：プリセット保存は現在のみ更新、別名保存はプリセットのみバックアップしセーブの紐づけは変えず、新規保存は現在のセーブを複製し専用プリセットを作ります。バックアップはセーブ・キャラ/シナリオプリセット・アイコンを保存（機種変更や復元用）、API キーは含みません。読み込み後もこの端末の UI 言語と外観は保持されます。"],
"成人模式（親密場景）：在狀態面板可開關（預設關）。開啟後遊戲會調整提示詞，讓 AI 在角色皆為成年人且合意的前提下更自然承接親密場景；實際尺度仍取決於所選模型。": ["Mature mode (intimate scenes): toggle it in the status panel (off by default). When on, the game adjusts the prompt so the AI more naturally handles intimate scenes on the premise that all characters are consenting adults; the actual explicitness still depends on the model you chose.", "成人モード（親密シーン）：ステータスパネルで切替（既定オフ）。オンにするとプロンプトを調整し、全員が成人かつ合意という前提で AI が親密シーンをより自然に受けます。描写度合いは選んだモデル次第です。"],
...
'刪除這則': ['Delete this entry', 'この保存を削除'],
...
'v2.2 更新': ['v2.2 Update', 'v2.2 更新'], 'v2.3 更新（07/08）': ['v2.3 Update (07/08)', 'v2.3 更新（07/08）'], 
...
'在日記的翻頁列（1／2）按右鍵、手機長按，可把整篇日記存成長圖；內容過長會自動分頁，且不會把對話或旁白切成兩半。': ['On the Diary page, right-click the page switcher (1／2), or long-press it on mobile, to save the whole entry as one long image; overly long entries split into multiple images automatically, without cutting any message or narration in half.', '日記ページのページ切り替え（1／2）を右クリック（スマホは長押し）すると、その日記全体を1枚の長い画像として保存できます。長すぎる場合は自動的に複数枚に分割され、会話やナレーションが途中で切れることはありません。'],
...
'新增低 HP／低 SAN 干擾：狀態進入危機區間時，畫面會出現暗角、顆粒、掃描線、短暫亂碼與狀態訊號，且配色會跟著目前介面主題變化。': ['Added low-HP / low-SAN interference: when survival status enters the crisis range, the screen shows vignette, grain, scan lines, brief text corruption, and status signals, with colors following the current UI theme.', '低HP／低SAN干渉を追加：生存状態が危機域に入ると、画面に暗い縁取り、粒子、走査線、一時的な文字化け、状態シグナルが現れ、配色は現在のUIテーマに連動します。'], '低 SAN 選項會變得不可靠：點選時可能變色並改成 AI 產生的隱藏失控選項，系統會直接送出並進行檢定。': ['Low-SAN options become unreliable: clicking one may change its color, turn into an AI-written hidden loss-of-control option, and immediately submit it with a check.', 'SANが低いと選択肢が不安定になります：クリックした瞬間に色が変わり、AIが生成した非表示の暴走選択肢に書き換わり、その内容が判定つきで送信されることがあります。'], 'HP 與 SAN 代表角色的生存狀態。HP 或 SAN 低於 20 時會進入危機區間，部分判定會受影響；畫面也會出現暗角、底片顆粒、掃描線、短暫文字亂碼與狀態訊號，但配色會跟著目前介面主題走。': ['HP and SAN represent your character’s survival state. When HP or SAN drops below 20, the character enters a crisis range and some checks are affected; the screen also gains vignette, film grain, scan lines, brief text corruption, and status signals, all colored by the current UI theme.', 'HPとSANはキャラクターの生存状態を表します。HPまたはSANが20未満になると危機域に入り、一部の判定に影響します。画面にも暗い縁取り、フィルム粒子、走査線、一時的な文字化け、状態シグナルが現れますが、配色は現在のUIテーマに合わせられます。'], 'SAN 很低時，AI 提供的選項可能變得不可靠。AI 會額外準備一個不顯示的隱藏失控選項；污染觸發時，被點選的選項會在瞬間變色、文字改成那個隱藏行動，並立刻送出與進行檢定；如果沒有觸發污染，點選仍會像平常一樣先填入輸入框。': ['When SAN is very low, AI options can become unreliable. The AI prepares one hidden loss-of-control option; if corruption triggers, the clicked option flashes, rewrites itself into that hidden action, and immediately submits it with a check. If corruption does not trigger, clicking still fills the input field as usual.', 'SANが非常に低いと、AIの選択肢が不安定になることがあります。AIは非表示の暴走選択肢を1つ用意します。汚染が発生すると、クリックした選択肢は瞬間的に色が変わり、その隠れた行動へ書き換わって、判定つきですぐ送信されます。汚染が発生しなかった場合は、通常どおり入力欄に入るだけです。'], '新增對話右鍵／手機長按選單：可編輯已送出的文字、複製對話，或把片段收藏到日記。': ['Added message right-click / mobile long-press menu: edit sent text, copy dialogue, or add a scene to Diary.', '発言の右クリック／スマホ長押しメニューを追加：送信済みテキストの編集、会話のコピー、場面を日記に保存できます。'],
...
'進入遊戲後，直接在下方輸入角色想說的話或想做的事，也可以點選 AI 提供的選項。若行動具有風險，可以使用骰子按鈕進行 D20 判定；系統會依角色能力、遊戲難度與骰點決定成功或失敗。想修正已送出的文字、複製對話或收藏片段時，桌機可在對話上按右鍵，手機可長按，開啟小選單。': ['In game, type what your character says or does, or choose an option from the AI. Risky actions can use the dice button for a D20 check; success or failure depends on character ability, game difficulty, and the roll. To fix sent text, copy dialogue, or save a scene, right-click a message on desktop or long-press on mobile to open the small menu.', 'ゲーム中は下の欄にキャラクターの台詞や行動を入力するか、AIの選択肢を選べます。危険な行動はダイスボタンでD20判定を行い、能力値・難易度・出目で成否が決まります。送信済みの文字を直す、会話をコピーする、場面を保存する場合は、PCでは発言を右クリック、スマホでは長押しして小さなメニューを開きます。'],
'角色面板可以查看與修改玩家狀態、NPC、道具、Flags、任務、劇情摘要及角色關係；AI 每回合會參考這些重點資料來延續故事。冒險日誌記錄完整事件，日記則用來收起想保留的對話片段；在對話泡泡上按右鍵（手機長按）選「收藏這段」，再點另一則當結尾，就會把連續對話收進日記。': ['The character panel lets you view and edit player status, NPCs, items, Flags, tasks, story summaries, and relationships; the AI uses these notes each turn to continue the story. Adventure Log keeps full events, while Diary saves dialogue scenes you want to keep. Right-click a dialogue bubble (long-press on mobile) and choose "Add to Diary", then click another message as the end to save the continuous dialogue into Diary.', 'キャラクターパネルでは、プレイヤー状態、NPC、アイテム、Flags、タスク、物語の要約、関係性を確認・編集できます。AIは毎ターンこれらを参照して物語を続けます。冒険日誌は出来事全体を記録し、日記は残しておきたい会話場面を保存します。会話の吹き出しを右クリック（スマホは長押し）して「日記に追加」を選び、終点にしたい発言をもう一つ選ぶと、連続した会話が日記に保存されます。'],
...
'查看更早的更新紀錄': ['View earlier update history', '以前の更新履歴を見る'],
        ...
        '💡 AI 忘記、重複或場景錯亂怎麼辦？': ['💡 AI forgetting, repeating, or confusing scenes?', '💡 AIが忘れる・繰り返す・場面が混乱するときは？'],
        '先打開角色面板，檢查重點劇情、任務、角色關係、Flags 與動態狀態，可手動修正或用「整理摘要／整理紀錄」重新整理，並確認冒險日誌是否正確。如果 AI 仍然混淆，可開啟「神」模式，直接說明目前場景、在場角色與接下來想聚焦的方向。': ['First open the character panel and check the story highlights, tasks, relationships, Flags, and dynamic states; fix them manually or use Organize Summary / Organize Log, and confirm the adventure log is correct. If the AI is still confused, use God (creator) mode to state the current scene, who is present, and what to focus on next.', 'まずキャラクターパネルを開き、重要イベント、タスク、関係、Flags、動的状態を確認します。手動で修正するか「要約整理／記録整理」で整理し、冒険日誌が正しいか確認してください。それでもAIが混乱する場合は「神」モードで、現在の場面、登場人物、次に集中したい方向を直接伝えましょう。'],
        '💡 重大事件與里程碑會自動記錄': ['💡 Major events and milestones are recorded automatically', '💡 重要な出来事とマイルストーンは自動で記録されます'],
        '當 NPC 好感度達到滿值、與你關係徹底破裂，或在劇情中死亡時，系統會自動在 Flags 留下一條里程碑標籤，不必依賴 AI 自行判斷。此外，即使 AI 當回合沒有特別描述，獲得／失去重要道具或場景移動也會自動補進冒險日誌——重大關係與事件因此不會被漏記。': ['When an NPC reaches max affection, your relationship fully breaks down, or they die in the story, the system automatically leaves a milestone flag in Flags, without relying on the AI to decide. In addition, even if the AI does not describe it that turn, gaining or losing key items or a location change is added to the adventure log automatically — so major relationships and events are not missed.', 'NPCの好感度が最大に達したとき、関係が完全に決裂したとき、または物語中で死亡したとき、システムがAIの判断に頼らず自動的にFlagsへマイルストーンフラグを残します。さらに、AIがそのターンで特に描写しなくても、重要アイテムの入手や喪失、場所の移動が自動的に冒険日誌へ追加されるため、重要な関係や出来事が漏れません。'],
        '即使規則調得再細，故事拉長後 AI 仍可能變遲鈍或銜接不順，這是模型本身的能力限制。建議：（一）長篇、複雜或重視連貫的故事，盡量選較強的模型（清單中標 ▪ 的大廠優質模型，例如 Gemini Pro／Flash、Claude Sonnet／Opus、GPT-4.1）；輕量模型（Lite、Haiku）雖然快又省，但較容易忘記與跳針。（二）善用「整理摘要／整理紀錄」壓縮記憶，並把關鍵設定寫進「核心準則」或角色面板。（三）模型可在遊戲中隨時切換，不必重開。': ['Even with carefully tuned rules, the AI may become dull or lose continuity as a story grows long; this is a limit of the model itself. Tips: (1) For long, complex, or continuity-heavy stories, prefer a stronger model (the ▪ premium general models in the list, e.g., Gemini Pro/Flash, Claude Sonnet/Opus, GPT-4.1); lightweight models (Lite, Haiku) are fast and cheap but forget and lose the thread more easily. (2) Use Organize Summary / Organize Log to compress memory, and put key facts into Core Rules or the character panel. (3) You can switch models any time during play, no need to restart.', 'ルールを細かく調整しても、物語が長くなるとAIは鈍くなったり話がつながらなくなることがあります。これはモデル自体の限界です。おすすめ：（一）長編・複雑・連続性重視の物語では、なるべく強いモデル（一覧で▪の大手汎用高品質モデル、例：Gemini Pro／Flash、Claude Sonnet／Opus、GPT-4.1）を選びましょう。軽量モデル（Lite、Haiku）は速くて安いですが、忘れやすく話が飛びやすいです。（二）「要約整理／記録整理」で記憶を圧縮し、重要な設定は「コアルール」やキャラクターパネルに書いておきましょう。（三）モデルはプレイ中いつでも切り替えられ、やり直す必要はありません。'],
...
'💡 核心準則（AI 不可違反的鐵則）': ['💡 Core Rules (rules the AI must never break)', '💡 コアルール（AIが破れない絶対ルール）'],
'角色設定頁與遊戲頁都有「核心準則」小標籤。寫進去的設定（例如世界觀「這世界沒有魔法」，或人物鐵律「主角必須一直戴著面具」），AI 每回合都會優先讀取、不會遺忘也不可違反。它跟著配置保存，兩邊同步。': ['Both the character config page and the game page have a small "Core Rules" tab. Whatever you write there (for example a world fact like "there is no magic in this world," or a character rule like "the protagonist must always wear a mask") is read first every turn and can never be forgotten or broken. It is saved with the config and stays in sync across both places.', 'キャラクター設定ページとゲーム画面の両方に「コアルール」の小さなタブがあります。そこに書いた設定（例：「この世界に魔法はない」という世界観や、「主人公は常に仮面をつけている」というキャラクターの決まり）は毎ターン最優先で読まれ、忘れられることも破られることもありません。設定と一緒に保存され、両方で同期されます。'],
'💡 玩久了 AI 變笨、接不上怎麼辦？': ['💡 What if the AI gets dull or loses continuity after long play?', '💡 長くプレイするとAIが鈍くなる・話がつながらないときは？'],
'即使規則調得再細，故事拉長後 AI 仍可能變遲鈍或銜接不順，這是模型本身的能力限制。建議：（一）長篇、複雜或重視連貫的故事，盡量選較強的模型（清單中標 ★ 的優先項，例如 Gemini Pro／Flash、Claude Sonnet／Opus、GPT-4.1）；輕量模型（Lite、Haiku）雖然快又省，但較容易忘記與跳針。（二）善用「整理摘要／整理紀錄」壓縮記憶，並把關鍵設定寫進「核心準則」或角色面板。（三）模型可在遊戲中隨時切換，不必重開。': ['Even with carefully tuned rules, the AI may become dull or lose continuity as a story grows long; this is a limit of the model itself. Tips: (1) For long, complex, or continuity-heavy stories, prefer a stronger model (the star picks in the list, e.g., Gemini Pro/Flash, Claude Sonnet/Opus, GPT-4.1); lightweight models (Lite, Haiku) are fast and cheap but forget and lose the thread more easily. (2) Use Organize Summary / Organize Log to compress memory, and put key facts into Core Rules or the character panel. (3) You can switch models any time during play, no need to restart.', 'ルールを細かく調整しても、物語が長くなるとAIは鈍くなったり話がつながらなくなることがあります。これはモデル自体の限界です。おすすめ：（一）長編・複雑・連続性重視の物語では、なるべく強いモデル（一覧の★の優先項目、例：Gemini Pro／Flash、Claude Sonnet／Opus、GPT-4.1）を選びましょう。軽量モデル（Lite、Haiku）は速くて安いですが、忘れやすく話が飛びやすいです。（二）「要約整理／記録整理」で記憶を圧縮し、重要な設定は「コアルール」やキャラクターパネルに書いておきましょう。（三）モデルはプレイ中いつでも切り替えられ、やり直す必要はありません。'],
'💡 本場目標（給情境一個方向）': ['💡 Scene Goal (give a scenario a direction)', '💡 この場の目標（シナリオに方向性を）'],
'情境設定裡每個情境可選填「本場目標」。填了之後 DM 會讓劇情自然朝目標推進、鋪陳線索與機會，但不會硬拉玩家或無視你的選擇；留空則自由發展。': ['In Scenario settings, each scenario can have an optional "Scene Goal." When set, the DM steers the story toward it naturally, laying out clues and opportunities, without forcing the player or ignoring your choices; leave it blank for free-form play.', 'シナリオ設定では、各シナリオに任意の「この場の目標」を設定できます。設定すると、DMは手がかりや機会を用意しながら自然にその目標へ物語を導きます（プレイヤーを強制したり選択を無視したりはしません）。空欄なら自由に展開します。'],
...
'送給 AI 的背景資料太長，請先用狀態面板的「整理摘要／整理紀錄」壓縮後再試。': ['The background sent to the AI is too long. Use “Organize Summary / Organize Log” in the status panel to compress it first.', 'AIに送る背景情報が長すぎます。ステータスパネルの「要約整理／記録整理」で圧縮してから再試行してください。'],
...
'依角色生成擅長': ['Generate from character', 'キャラから生成'],
'角色專屬的魔法': ['Character-only magic', 'このキャラだけの魔法'],
'顯示與輸出設定': ['Display & Output', '表示と出力'],
'介面與 AI 輸出的語言，以及每回合回覆的長度。': ['Language for the UI and AI output, plus how long each reply is.', 'UIとAI出力の言語、そして毎回の返信の長さ。'],
'回覆長度': ['Reply length', '返信の長さ'],
        ...
        '💡 成人模式（親密場景）': ['💡 Mature Mode (intimate scenes)', '💡 成人モード（親密な場面）'],
        '開啟後，遊戲會調整提示詞，讓 AI 在角色皆為成年人且合意的前提下，更自然承接親密場景；實際尺度仍取決於所選模型。': ["When on, the game adjusts its prompts so the AI picks up intimate scenes more naturally, on the premise that all characters are adults and consenting; the actual explicitness still depends on the model you choose.", 'オンにすると、ゲームがプロンプトを調整し、登場人物が全員成人かつ合意しているという前提のもとで、AIが親密な場面をより自然に受け止めます。実際の表現の程度は、選んだモデルによって変わります。'],
        '故事拉長後，AI 可能變遲鈍或接不上，這是模型本身的能力限制。三個改善方向：': ['As a story grows long, the AI may get dull or lose the thread — this is a limit of the model itself. Three ways to improve it:', '物語が長くなると、AIは鈍くなったり話がつながらなくなることがあります。これはモデル自体の限界です。改善の3つの方向：'],
        '・選強一點的模型：長篇／重視連貫選標 ▪ 的大廠款（Gemini Pro／Flash、Claude Sonnet／Opus、GPT-4.1）；輕量款（Lite、Haiku）快省但容易忘、跳針。': ['・Pick a stronger model: for long or continuity-heavy stories, choose the ▪ premium models (Gemini Pro/Flash, Claude Sonnet/Opus, GPT-4.1); lightweight ones (Lite, Haiku) are fast and cheap but forget and repeat more easily.', '・強めのモデルを選ぶ：長編や連続性重視なら▪の大手モデル（Gemini Pro／Flash、Claude Sonnet／Opus、GPT-4.1）。軽量モデル（Lite、Haiku）は速くて安いですが忘れやすく話が飛びやすいです。'],
        '・壓縮記憶：多用「整理摘要／整理紀錄」，關鍵設定寫進「核心準則」或角色面板。': ['・Compress memory: use Organize Summary / Organize Log often, and put key facts into Core Rules or the character panel.', '・記憶を圧縮：「要約整理／記録整理」をこまめに使い、重要な設定は「コアルール」やキャラクターパネルに書いておきましょう。'],
        '・隨時換模型：遊戲中可切換，不必重開。': ['・Switch anytime: you can change models mid-game without restarting.', '・いつでも切替：ゲーム中にモデルを切り替えられ、やり直し不要です。'],
...
'每份遊戲紀錄只綁定一份配置。儲存配置只更新目前配置；另存配置只備份配置，不會改綁遊戲紀錄；另存新檔會複製目前遊戲紀錄並建立專屬配置。': ['Each game save binds to one preset. Save Preset updates only the current preset; Save Preset As only backs up the preset and does not rebind a game save; Save New File copies the current game save and creates its dedicated preset.', '各ゲームデータは1つの設定だけに紐づきます。設定を保存は現在の設定だけを更新します。設定を別名保存は設定のバックアップのみで、ゲームデータの紐づけは変えません。新規データとして保存は現在のゲームデータを複製し、専用設定を作成します。'],
'每份遊戲紀錄都會綁定當時使用的角色配置與情境配置。之後就算切換到其他配置，打開該紀錄時仍會使用它自己的綁定資料；如果在紀錄內另存配置，另存配置不會改綁該紀錄。匯入同一份備份時，系統會略過重複資料，避免產生多份相同紀錄。': ['Each save is bound to the character and scenario presets used at that time. Even if you later switch presets, opening that save uses its own bound data. Importing the same backup again skips duplicate data to avoid creating identical copies.', '各データは、その時点で使っていたキャラクター設定とシナリオ設定に紐づきます。あとで別の設定に切り替えても、そのデータを開くと自身の紐づけ情報が使われます。同じバックアップを再インポートした場合、重複データはスキップされ、同じ記録が複数作られないようにします。'],
'9. 備份包含與不包含的資料': ['9. What Backups Include', '9. バックアップに含まれるもの'],
'匯出記憶紀錄會保存遊戲紀錄、角色配置、情境配置與角色頭像，方便換裝置或恢復進度。備份不包含 API Key。匯入備份後，會保留目前裝置原本的介面語言與外觀設定。': ['Exported data saves game records, character presets, scenario presets, and character avatars, making it useful for moving devices or restoring progress. API keys are not included. After importing, the current device’s interface language and appearance settings are preserved.', 'データのエクスポートでは、ゲームデータ、キャラクター設定、シナリオ設定、キャラクター画像が保存され、端末移行や進行状況の復元に使えます。APIキーは含まれません。インポート後も、現在の端末の表示言語と外観設定は維持されます'],
...
'5. 玩家、旁白與「神」模式': ['5. Player, Narrator & Creator Mode', '5. プレイヤー、ナレーター、「神」モード'],
        ...
        '每份遊戲紀錄都會綁定目前使用的角色配置，彼此的摘要、任務、關係與冒險日誌會分開保存。另存配置只備份配置，不會改綁遊戲紀錄；另存新檔會複製目前遊戲紀錄並建立專屬配置。上鎖配置不會被遊戲內修改覆蓋。資料主要保存在目前裝置的瀏覽器，不會自動同步，請定期在存檔選單使用「匯出」備份，並在更換裝置後使用「匯入」恢復。': ['Each save is bound to its current character preset, with separate summaries, tasks, relationships, and logs. Save Preset As only backs up the preset and does not rebind a game save. Save New File copies the current game save and creates its dedicated preset. Locked presets cannot be overwritten. Data is local to this browser, so export backups regularly and import them on a new device.', '各データは現在のキャラクター設定に紐づき、要約、タスク、関係、日誌は別々に保存されます。設定を別名保存は設定のバックアップのみで、ゲームデータの紐づけは変えません。新規データとして保存は現在のゲームデータを複製し、専用設定を作成します。ロック中の設定は上書きされません。定期的にエクスポートし、新しい端末でインポートしてください。'],
 ...
 '1. API Key 是什麼？': ['1. What Is an API Key?', '1. APIキーとは？'],
 'API Key 是 AI 服務用來辨識使用者與計算用量的金鑰，不是遊戲帳號或遊戲密碼。請向 Google AI Studio 或 OpenRouter 申請，並妥善保管；不要把金鑰貼到公開貼文、截圖或傳給其他人。': ['An API key is used by an AI service to identify the user and calculate usage. It is not a game account or game password. Apply for one from Google AI Studio or OpenRouter, keep it safe, and do not post it publicly, include it in screenshots, or send it to other people.', 'APIキーは、AIサービスが利用者を識別し、使用量を計算するための鍵です。ゲームのアカウントやパスワードではありません。Google AI StudioまたはOpenRouterで取得し、大切に保管してください。公開投稿やスクリーンショットに載せたり、他人に送ったりしないでください。'],
 '2. Google Gemini 與 OpenRouter': ['2. Google Gemini and OpenRouter', '2. Google GeminiとOpenRouter'],
 '選擇 Google Gemini 時，請使用 Google AI Studio 提供的金鑰；選擇 OpenRouter 時，請使用 OpenRouter 金鑰。兩種金鑰不能互換。OpenRouter 可提供多家模型，Gemini 則直接使用 Google 模型；實際可用清單會在驗證後載入。': ['When choosing Google Gemini, use a key from Google AI Studio. When choosing OpenRouter, use an OpenRouter key. The two key types are not interchangeable. OpenRouter can provide models from multiple providers, while Gemini uses Google models directly. The available model list loads after verification.', 'Google Geminiを選ぶ場合はGoogle AI Studioのキーを使ってください。OpenRouterを選ぶ場合はOpenRouterのキーを使ってください。2種類のキーは互換性がありません。OpenRouterでは複数社のモデルを利用でき、GeminiではGoogleモデルを直接利用します。実際に使えるモデル一覧は認証後に読み込まれます。'],
 '3. 模型怎麼選？': ['3. How Should I Choose a Model?', '3. モデルの選び方'],
 '輕量模型通常回覆較快、用量較省，適合測試設定或日常遊玩；較大型模型通常更擅長長篇劇情、人物語氣與複雜指令。清單中的 ★ 代表較適合目前遊戲用途的優先選項。模型表現與費用可能不同，請以供應商顯示的資訊為準。': ['Lightweight models usually respond faster and use less quota, making them suitable for testing settings or everyday play. Larger models are usually better at long stories, character voice, and complex instructions. A ★ in the list marks options recommended for this game. Model performance and cost may vary; check the provider information.', '軽量モデルは通常、応答が速く使用量も少ないため、設定テストや日常プレイに向いています。大型モデルは長編ストーリー、キャラクターの口調、複雑な指示を扱いやすい傾向があります。一覧の★は、このゲーム用途に比較的向いている優先候補です。モデルの性能や費用は異なる場合があるため、プロバイダー側の表示を確認してください。'],
