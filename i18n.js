(function () {
    ...
    const M = {
"完成 API 驗證並選好模型後，建立角色與情境、進入存檔，輸入第一個行動就能開始。（金鑰設定、模型選擇與錯誤排除請看首頁的 API 說明。）": ["After verifying your API key and choosing a model, create your characters and scenario, enter a save, and type your first action to begin. (See the API guide on the home page for key setup, model selection, and troubleshooting.)", "API キーを認証してモデルを選んだら、キャラクターとシナリオを作成し、データに入って最初の行動を入力すれば開始です。（キー設定・モデル選択・トラブル対処はホームの API 説明を参照してください。）"],
"第一次跑團時，可以在進入遊戲後使用「遊戲教學」功能。教學會直接在目前的角色與情境中啟動，依序帶你完成自由輸入、擲骰、使用道具與好感互動，不需要改用其他預設配置。完成後會繼續目前的故事；若流程卡住，最晚會在 16 個旁白回合後自動結束。": ["First time playing? After entering the game, use the \"Game Tutorial\" feature. It starts inside your current characters and scenario and walks you through free input, dice rolls, item use, and affection interactions, with no need to switch to another preset. When it finishes, your current story continues; if the flow gets stuck, the tutorial ends automatically after at most 16 narrator turns.", "初めて遊ぶ場合は、ゲームに入ったあと「ゲームチュートリアル」機能を使用できます。現在のキャラクターとシナリオ内で直接始まり、自由入力・ダイス・アイテム使用・好感度のやり取りを順に案内するため、別のプリセットへ切り替える必要はありません。完了後は現在の物語を続行し、進行が止まった場合もナレーター16ターン以内に自動終了します。"],
"在「劇本創角」裡設定玩家角色、NPC、情境與遊戲選項；角色與情境也能交給 AI 隨機生成。": ["In \"Character Setup\", configure the player character, NPCs, scenarios, and game options; the AI can also generate characters and scenarios for you.", "「キャラクター作成」でプレイヤーキャラクター・NPC・シナリオ・ゲーム設定を行います。キャラクターとシナリオは AI にランダム生成させることもできます。"],
"配置總覽可以鎖定想保留的配置。批次選擇與批次刪除只會處理未鎖定的配置，避免誤刪重要角色。": ["You can lock presets you want to keep in the preset overview. Bulk selection and deletion only affect unlocked presets, helping protect important characters from accidental deletion.", "設定一覧では、残したい設定をロックできます。一括選択と一括削除はロックされていない設定だけを対象にするため、大切なキャラクターの誤削除を防げます。"],
"NPC 除了直接編輯，也能使用「認識夥伴」：先填寫至少一項人物基底，再透過預設問題或自己的問題和 NPC 對話。AI 的回答、玩家的提問與補充不會直接改動正式資料；只有在最後確認內容、選擇欄位並決定追加或取代後，才會保存到 NPC 設定。": ["Besides editing an NPC directly, you can use \"Meet This Companion\": fill in at least one character detail, then talk with the NPC through a suggested question or one of your own. AI replies, player questions, and follow-ups never change the saved data automatically; the NPC is updated only after you confirm the final text, choose a field, and decide whether to append or replace it.", "NPC は直接編集するほか、「仲間を知る」も利用できます。人物の基礎項目を1つ以上入力し、用意された質問または自分の質問で NPC と会話します。AI の回答、プレイヤーの質問や補足が正式データを自動変更することはありません。最後に内容を確認し、保存先の項目と追記／置換を選んだ場合だけ NPC 設定へ保存されます。"],
"核心準則是 AI 每回合優先讀取、不可違反的鐵則。適合固定世界觀、人物設定，或鎖定場景，例如寫下「B 目前不在此情境」，避免不在場的 NPC 突然出現。": ["Core Rules are iron rules the AI reads first every turn and must not break. Use them to fix the world, character details, or a scene—for example, write \"B is not in this scene\" to keep an absent NPC from suddenly appearing.", "コアルールは AI が毎ターン最優先で読み、破ってはならない鉄則です。世界観・人物設定・場面の固定に使えます。たとえば「B は今このシナリオにいない」と書けば、不在の NPC が突然現れるのを防げます。"],
"每個情境都能設定「本場目標」。DM 會自然朝目標推進，達成時觸發章節收束；留空則讓故事自由發展。": ["Each scenario can have a \"Scene Goal\". The GM steers toward it naturally, and reaching it triggers a chapter close; leave it blank for free-form play.", "各シナリオには「本場の目標」を設定できます。GM は自然に目標へ導き、達成すると章の締めくくりが発生します。空欄なら物語は自由に展開します。"],
"在輸入框輸入角色想說或想做的事，也可以直接選擇 AI 提供的行動。": ["Type what your character wants to say or do, or choose an action offered by the AI.", "入力欄にキャラクターのセリフや行動を入力するか、AI が提示した行動をそのまま選びます。"],
"行動選項會標示可能使用的屬性與難度，例如「觀察・WIS」。符合角色擅長領域的行動會顯示「★熟練」，玩家角色進行該項檢定時獲得 +2。": ["Action options show the likely stat and difficulty, such as \"Observe · WIS\". Actions matching the character's proficiencies display \"★ Proficient\", granting the player character +2 on that check.", "行動選択肢には「観察・WIS」のように、使用する可能性のある能力値と難易度が表示されます。キャラクターの得意分野に合う行動には「★得意」が表示され、その判定でプレイヤーキャラクターに +2 が加わります。"],
"有風險的行動可使用骰子按鈕進行 D20 判定。相同難度的基礎 DC 會在指定區間內浮動，再加上遊戲難度與角色狀態修正；最終結果由遊戲程式計算，AI 不會改寫骰點。": ["Use the dice button for a D20 check on risky actions. The base DC varies within a range for each difficulty, then game difficulty and character-state modifiers are applied. The game calculates the final result; the AI cannot rewrite the roll.", "危険な行動はダイスボタンで D20 判定を行えます。同じ難易度でも基礎 DC は指定範囲内で変動し、さらにゲーム難易度とキャラクター状態の補正が加わります。最終結果はゲームが計算し、AI が出目を書き換えることはありません。"],
"想安排環境、鏡頭或讓 NPC 行動，切換輸入框上方的「旁白」籤；點「行動」回到玩家角色。想直接改變場景、時間或人物狀態，切換到「神」籤。旁白與神模式的內容都不會被當成玩家角色的發言或行動。": ["To arrange the environment, camera, or NPC actions, switch to the \"Narrator\" tab above the input box; choose \"Action\" to return to the player character. To directly change the scene, time, or character state, switch to \"Creator\". Narrator and Creator input never counts as the player character's speech or action.", "環境・カメラ・NPC の行動を指定するには入力欄上の「ナレーター」タブへ切り替え、「行動」でプレイヤーキャラクターに戻ります。場面・時間・人物の状態を直接変える場合は「神」タブへ切り替えます。ナレーターと神モードの内容は、プレイヤーキャラクターの発言や行動として扱われません。"],
"已送出的文字需要修改、複製或收藏時，桌機使用右鍵，手機使用長按開啟選單。": ["To edit, copy, or save already-sent text, right-click on desktop or long-press on mobile to open the menu.", "送信済みの文章を修正・コピー・保存する場合は、PC では右クリック、スマホでは長押しでメニューを開きます。"],
"HP 或 SAN 偏低時會進入危機狀態：檢定變難，畫面也會出現暗角、雜訊與狀態訊號，越接近歸零越強烈。": ["Low HP or SAN puts you in crisis: checks become harder, and vignetting, noise, and status signals intensify as you approach zero.", "HP または SAN が低いと危機状態に入り、判定が難しくなります。画面のビネット・ノイズ・状態シグナルも、ゼロに近づくほど強くなります。"],
"如果覺得干擾太多，可在角色面板「系統」頁的「顯示與輸出設定 → 低狀態干擾」切換為完整、減弱或關閉。": ["If the effects feel excessive, open the character panel's \"System\" tab and set \"Display & Output → Low HP/SAN Effects\" to Full, Reduced, or Off.", "演出が多すぎる場合は、キャラクターパネルの「システム」ページで「表示と出力 → 低HP／SAN演出」をフル・控えめ・オフから選べます。"],
"SAN 很低時，AI 提供的選項可能受到污染。點選後有機率變成隱藏的失控行動，並直接送出進行檢定。": ["At very low SAN, AI-provided options may become corrupted. Selecting one can turn it into a hidden loss-of-control action that is sent immediately for a check.", "SAN が非常に低いと、AI の選択肢が汚染されることがあります。選ぶと隠れた制御喪失行動に変わり、そのまま送信されて判定に入る場合があります。"],
"標準模式下，玩家 HP 或 SAN 歸零不會結束遊戲，系統會將狀態保護在 1 點。": ["In Standard mode, reaching zero HP or SAN does not end the game; the system protects the stat at 1.", "標準モードでは、プレイヤーの HP または SAN がゼロになってもゲームは終了せず、システムが 1 に保護します。"],
"困難模式下，玩家歸零時必須通過生死檢定；NPC 只有一次復活機會。": ["In Hard mode, the player must pass a life-or-death check at zero; each NPC has only one chance to revive.", "困難モードでは、プレイヤーがゼロになると生死判定に成功する必要があります。NPC の蘇生機会は1回だけです。"],
"極限模式的判定最嚴格，NPC 死亡後無法復活。玩家歸零時會進入「最後掙扎」，通過生死檢定才能撐住，並獲得幾回合的恢復期保護。": ["Extreme mode has the harshest checks, and dead NPCs cannot be revived. When the player reaches zero, \"Last Stand\" begins; pass the life-or-death check to hold on and gain a few turns of recovery protection.", "極限モードは判定が最も厳しく、死亡した NPC は復活できません。プレイヤーがゼロになると「最後の抵抗」に入り、生死判定に成功すれば踏みとどまり、数ターンの回復保護を得ます。"],
"輸入列的杯子圖示是「喘息」。使用後會進行一次檢定，成功可回復部分 HP／SAN，用來脫離死亡螺旋；回復上限為總量的 50%，而且不能連續使用。": ["The cup icon on the input row is \"Catch Breath\". It makes a check and restores some HP/SAN on success, helping you escape a death spiral; recovery is capped at 50% of the maximum and cannot be used consecutively.", "入力欄のカップアイコンは「ひと息」です。判定に成功すると HP／SAN を一部回復し、死のスパイラルから抜け出せます。回復上限は最大値の 50% で、連続使用はできません。"],
"部分劇情道具有回復效果，可在角色面板的道具區按「使用」。何時使用由玩家自行決定，一般敘事與 NPC 不會反覆催促或後設指示介面操作。": ["Some story items have recovery effects and can be activated with \"Use\" in the character panel's item area. You decide when to use them; ordinary narration and NPCs will not repeatedly prompt you or give meta instructions about the interface.", "一部の物語アイテムには回復効果があり、キャラクターパネルの所持品欄で「使う」を押せます。使用するタイミングはプレイヤーが決めます。通常の叙述や NPC が繰り返し催促したり、画面操作をメタ的に指示したりすることはありません。"],
"充分休息、過夜、接受治療或受到安撫時，AI 也能依事件小幅回復 HP／SAN，但不能靠重複休息無限回復。": ["Proper rest, sleeping overnight, receiving treatment, or being comforted can also restore a small amount of HP/SAN when the event supports it, but repeated resting cannot provide unlimited recovery.", "十分な休息・一晩の睡眠・治療・慰めなど、出来事に応じて AI が HP／SAN を少し回復させることもあります。ただし休息を繰り返して無限に回復することはできません。"],
"名稱帶有屬性標記的裝備道具，例如「幸運硬幣（智力+1）」，持有期間會自動加入對應骰點；失去道具後加值立即失效。": ["Equipment whose name includes a stat tag, such as \"Lucky Coin (INT+1)\", automatically adds its bonus to matching rolls while held; the bonus ends immediately if the item is lost.", "「幸運のコイン（知力+1）」のように名前へ能力タグが付いた装備は、所持中に対応する判定へ自動加算されます。アイテムを失うとボーナスは即座に無効になります。"],
"達成重大成就會獲得成長點，可在「角色成長」區提升屬性或新增擅長領域；角色稱號也會隨累積成就逐步升階。": ["Major achievements grant growth points that can raise stats or add proficiencies in \"Character Growth\"; the character's title also ranks up as achievements accumulate.", "重大な実績を達成すると成長ポイントを獲得し、「キャラ成長」で能力値を上げたり得意分野を追加したりできます。実績の累積に応じて称号も段階的に昇格します。"],
"屬性達到 20 後，該屬性的提升按鈕會改為鍛造：花費 2 點成長點製作「成長結晶」，讓該屬性判定 +1。成長結晶屬於道具，與其他裝備共用 +3 加值上限，也可能在劇情中失去。": ["When a stat reaches 20, its upgrade button becomes a forge action: spend 2 growth points to create a \"Growth Crystal\" that grants +1 to that stat's checks. Growth Crystals are items, share the +3 bonus cap with other equipment, and can be lost in the story.", "能力値が 20 に達すると、その強化ボタンは鍛造に変わります。成長ポイントを 2 消費して「成長結晶」を作ると、その能力の判定に +1。成長結晶はアイテムで、ほかの装備と +3 の上限を共有し、物語の中で失う場合もあります。"],
"達成情境的「本場目標」會觸發章節收束，由 AI 寫下收尾敘事並給予成長點。": ["Reaching a scenario's \"Scene Goal\" triggers a chapter close, with the AI writing the ending beat and awarding a growth point.", "シナリオの「本場の目標」を達成すると章の締めくくりが発生し、AI が締めの叙述を書いて成長ポイントを付与します。"],
"NPC 好感滿值、關係破裂或死亡等重大節點，系統會自動記入 Flags 與冒險日誌。好感降到 −100 時，會解鎖特殊成就「我們地獄見」。": ["Major milestones such as max NPC affection, a broken relationship, or death are recorded automatically in Flags and the adventure log. When affection falls to −100, the special achievement \"See You in Hell\" unlocks.", "NPC の好感度最大・関係決裂・死亡などの重要な節目は、Flags と冒険日誌へ自動記録されます。好感度が −100 まで下がると、特別な実績「地獄で会おう」が解除されます。"],
"角色面板上的像素愛心會隨好感改變動作：關係越好越活潑，滿值時會發光跳躍；關係惡化時會變得沮喪，降到谷底則整顆倒下。": ["The pixel heart on the character panel changes with affection: it becomes livelier as the relationship improves and glows and leaps at maximum; when the relationship worsens it grows dejected, toppling over at rock bottom.", "キャラクターパネルのピクセルハートは好感度で動きが変わります。関係が良いほど元気になり、最大では光って跳ねます。関係が悪化すると落ち込み、どん底ではそのまま倒れます。"],
"冒險日誌用來保存完整事件、任務結果、成就與旅程紀錄。標記為「★」的重要紀錄在 AI 整理時會保留原文與原本位置，也可以使用「只看★」快速篩選。": ["The adventure log keeps full events, task outcomes, achievements, and journey records. Entries marked \"★\" remain verbatim and in their original positions during AI organization, and \"★ Only\" provides a quick filter.", "冒険日誌には出来事・任務結果・実績・旅の記録を保存します。「★」を付けた重要な記録は AI 整理時も原文と元の位置が保たれ、「★のみ」で素早く絞り込めます。"],
"日記用來收藏喜歡的對話片段。對訊息泡泡按右鍵或長按，選擇「收藏這段」後即可選取一段連續對話，並補上一句自己的備註。": ["The Diary is for saving favorite conversation excerpts. Right-click or long-press a message bubble, choose \"Add to Diary\", select a continuous range, and add a short note of your own.", "日記は気に入った会話を保存する場所です。発言の吹き出しを右クリックまたは長押しし、「日記に追加」を選んで連続した会話範囲を選択し、自分の一言メモを添えられます。"],
"在日記頁對翻頁列按右鍵，手機則使用長按，可以將整篇日記保存成長圖；內容過長時會自動分頁，不會把同一段對話或旁白從中切開。": ["On the Diary page, right-click the pager—or long-press it on mobile—to save the entire Diary as a long image. Long content is paginated automatically without splitting a dialogue or narration block in half.", "日記ページでページ送り部分を右クリック、スマホでは長押しすると、日記全体を長い画像として保存できます。内容が長い場合は自動で分割され、同じ会話やナレーションの途中では切れません。"],
"故事變長後，AI 可能因模型限制而遺忘細節或開始跳針。可以改用模型清單中標有「▪」的模型，使用「整理摘要／整理冒險日誌」壓縮記憶，或切換到「神」模式直接說明目前場景與重要狀態。": ["As a story grows, model limits may cause the AI to forget details or repeat itself. Try a model marked \"▪\", use \"Organize Summary / Organize Adventure Log\" to compress memory, or switch to \"Creator\" mode and state the current scene and important conditions directly.", "物語が長くなると、モデルの限界で AI が細部を忘れたり同じことを繰り返したりする場合があります。「▪」付きモデルへ変える、「要約整理／冒険日誌整理」で記憶を圧縮する、または「神」モードで現在の場面と重要な状態を直接伝えてください。"],
"每份遊戲紀錄都會保存當時的角色、NPC 與情境快照，不依賴目前選中的配置。另存新檔會複製目前進度並建立獨立紀錄，避免不同旅程互相影響。": ["Each save stores a snapshot of its characters, NPCs, and scenario and does not depend on the currently selected preset. \"Save as New\" copies the current progress into an independent record so separate journeys do not affect one another.", "各ゲームデータには、その時点のキャラクター・NPC・シナリオのスナップショットが保存され、現在選択中の設定には依存しません。「新規保存」は現在の進行を複製して独立した記録を作り、別々の旅が互いに影響しないようにします。"],
"匯出備份會保存遊戲紀錄、角色配置、情境配置與角色頭像，方便換裝置或還原。備份不包含 API Key，也不會覆蓋新裝置目前使用的外觀配色。": ["Exported backups include game records, character presets, scenario presets, and avatars for moving devices or restoring data. They never include API keys and do not overwrite the theme currently used on the new device.", "書き出したバックアップにはゲームデータ・キャラクター設定・シナリオ設定・キャラクター画像が含まれ、端末移行や復元に使えます。API キーは含まれず、新しい端末で現在使用しているテーマ配色も上書きしません。"],
"角色面板的「系統」頁可以切換模型、調整回覆長度與低狀態干擾，並查看 API 使用統計。實際費用與額度仍以所選 AI 供應商後台為準。": ["The character panel's \"System\" tab lets you switch models, adjust reply length and low-state effects, and review API usage. Actual charges and quotas are determined by the selected AI provider.", "キャラクターパネルの「システム」ページでは、モデルの切替、返答の長さと低状態演出の調整、API 使用量の確認ができます。実際の料金と利用上限は、選択した AI 事業者の管理画面で確認してください。"],
"成人模式也位於「系統」頁，預設為關閉。開啟後，AI 會在角色皆為成年人且彼此合意的前提下，更自然地承接親密情節；實際尺度仍取決於所選模型及供應商限制。": ["Mature mode is also in the \"System\" tab and is off by default. When enabled, the AI handles intimate scenes more naturally when all characters are consenting adults; actual explicitness still depends on the selected model and provider limits.", "成人モードも「システム」ページにあり、初期状態ではオフです。オンにすると、登場人物が全員成人で互いに合意している場合に、AI が親密な場面をより自然に続けます。実際の描写範囲は選択したモデルと事業者の制限によります。"],
...
"建立你的故事：角色、NPC、情境與規則": ["Build your story: character, NPCs, scenarios & rules", "物語を作る：キャラクター・NPC・シナリオ・ルール"],
"怎麼玩：行動、選項、擲骰與模式": ["How to play: actions, options, dice & modes", "遊び方：行動・選択肢・ダイス・モード"],
"生存與危機：HP／SAN": ["Survival & crisis: HP / SAN", "生存と危機：HP／SAN"],
"難度、死亡與恢復": ["Difficulty, death & recovery", "難易度・死亡・回復"],
"成長與里程碑": ["Growth & milestones", "成長とマイルストーン"],
"冒險日誌、日記與 AI 記憶": ["Adventure Log, Diary & AI memory", "冒険日誌・日記・AI の記憶"],
"存檔、備份與系統設定": ["Saves, backups & system settings", "データ・バックアップ・システム設定"],
'v2.7 更新（07/10）': ['v2.7 update (07/10)', 'v2.7 アップデート（07/10）'],
"v3.0 更新（07/22）": ["v3.0 update (07/22)", "v3.0 アップデート（07/22）"],
"角色與配置介面全面整理，玩家、NPC、情境的查看與編輯更加直覺，桌機與手機操作也更順暢。": ["Character and preset interfaces have been comprehensively reorganized, making player characters, NPCs, and scenarios easier to view and edit on both desktop and mobile.", "キャラクターと設定の画面を全面的に整理し、プレイヤーキャラクター・NPC・シナリオの閲覧と編集をより直感的にしました。PCでもスマートフォンでも快適に操作できます。"],
"新增 NPC「認識夥伴」：可以透過預設或自訂問題逐步認識角色，只有在最後確認後才會將內容保存到 NPC 設定。": ["Added \"Meet Companion\" for NPCs: use preset or custom questions to learn about a character step by step. Nothing is saved to the NPC's settings until you confirm at the end.", "NPCの「仲間を知る」を追加しました。用意された質問または自分の質問で少しずつ人物を知り、最後に確認した内容だけがNPC設定へ保存されます。"],
"遊戲設定與系統頁重新整理：配置可以鎖定，批次選擇與刪除不會影響已鎖定項目；模型、回覆長度、畫面效果與 API 資訊也更方便管理。": ["Game Settings and System pages have been reorganized. Presets can now be locked, and bulk selection or deletion will not affect locked items. Models, reply length, visual effects, and API information are also easier to manage.", "ゲーム設定とシステムページを整理しました。設定をロックでき、一括選択や一括削除でもロック済み項目は対象になりません。モデル、返答の長さ、画面効果、API情報も管理しやすくなりました。"],
"首頁、存檔、日記、冒險日誌等介面與操作體驗一併改善，排版更整齊，並提升不同螢幕尺寸下的使用體驗。": ["Improved the layout and controls across the home page, saves, Diary, and Adventure Log for a cleaner experience on different screen sizes.", "ホーム、データ、日記、冒険日誌などのレイアウトと操作性を改善し、さまざまな画面サイズで使いやすくしました。"],
"新手教學與敘事規則改善：教學會沿用目前的角色與情境，卡住時最晚於第 16 個旁白回合自動結束；一般敘事與 NPC 也不會再反覆提醒玩家操作介面或使用道具。": ["Improved the tutorial and narrative rules: the tutorial uses your current characters and scenario and ends automatically after at most 16 narrator turns if it gets stuck. Regular narration and NPCs will also stop repeatedly telling players to use items or operate the interface.", "チュートリアルと叙述ルールを改善しました。現在のキャラクターとシナリオをそのまま使用し、進行が止まった場合もナレーター16ターン以内に自動終了します。通常の叙述やNPCが、アイテム使用や画面操作を繰り返し促すこともありません。"],
...
'長按以重設': ['Hold to reset', '長押しでリセット'],
'確定要刪除這個項目嗎？': ['Delete this item?', 'この項目を削除しますか？'],
'刪除此標籤': ['Delete this flag', 'このフラグを削除'],
'刪除道具': ['Delete item', 'アイテムを削除'],
'刪除道具：{item}': ['Delete item: {item}', 'アイテムを削除：{item}'],
'刪除擅長領域': ['Delete proficiency', '得意分野を削除'],
'刪除這則': ['Delete this entry', 'この日記を削除'],
'確定要刪除這個情境嗎？': ['Delete this scenario?', 'このシナリオを削除しますか？'],
...
'此配置目前綁定 {count} 筆遊戲紀錄（{saveNames}）。刪除配置會一起刪除這些紀錄，確定要刪除嗎？': ['This preset is bound to {count} saves ({saveNames}). Deleting the preset will also delete those saves. Continue?', 'この設定は{count}件のデータ（{saveNames}）に紐づいています。設定を削除すると、それらのデータも削除されます。削除しますか？'],
...
'全選': ['Select All', 'すべて選択'],
'遊戲規則': ['Game Rules', 'ゲームルール'],
'配置名冊': ['Preset List', '設定一覧'],
'角色資料保存': ['Character Data Storage', 'キャラクターデータ保存'],
'困難模式：檢定 DC +2，NPC 每次死亡只有一次復活檢定，失敗後永久死亡。': ['Hard: checks are DC +2. Each NPC gets one revival check per death; failure is permanent.', 'ハード：判定DC＋2。NPCは死亡ごとに復活判定が1回だけあり、失敗すると永久に死亡します。'],
'極限模式：檢定 DC +3，歸零會觸發「最後掙扎」，生死檢定 D20 ≥ 8 才能存活；NPC 死亡後無法復活。': ['Nightmare: checks are DC +3. At zero, Last Stand triggers; survive on D20 ≥ 8. Dead NPCs cannot be revived.', '極限：判定DC＋3。ゼロになると「最後の抵抗」が発生し、D20が8以上なら生存。NPCは死亡後に復活できません。'],
...
'確定要刪除選取的配置嗎？': ['Delete the selected presets?', '選択した設定を削除しますか？'],
...
'+ 新增情境': ['+ Add Scenario', '+ シナリオ追加'], '儲存變更': ['Save', '保存'], '另存新檔': ['Save New File', '新規データとして保存'], '返回選單': ['Back to Menu', 'メニューへ戻る'],
        ...
        '記憶紀錄': ['Saves', 'データ'], '本機資料狀態': ['Storage', 'ストレージ'], '重新計算': ['Refresh', '更新'],
        ...
        '修復': ['Repairs', '修復'], '顯示其餘 {count} 個': ['Show {count} more', '残り{count}件を表示'], '收合模型紀錄': ['Collapse model history', 'モデル履歴を折りたたむ'],
        '目前供應商：': ['Provider:\u00a0', 'プロバイダー：'], '目前模型：': ['Model:\u00a0', 'モデル：'], '最後使用：': ['Last used:\u00a0', '最終使用：'],
        '尚未選擇': ['Not selected', '未選択'], '尚未使用': ['Not used yet', '未使用'], '實際費用以所選 AI 供應商後台為準。': ['Check the selected AI provider for actual charges.', '実際の料金は選択したAIプロバイダーの管理画面でご確認ください。'], '重設統計': ['Reset Stats', '統計をリセット'],
        '確定要清除本機 API 使用統計嗎？遊戲存檔不會被刪除。': ['Reset local API usage stats? Game saves will not be deleted.', 'ローカルのAPI使用統計をリセットしますか？ゲームデータは削除されません。'],
...
'例如：\n• 玩家與 NPC 已建立信任\n• NPC 知道玩家正在承擔風險': ['Example:\n• Player and NPC have built trust\n• NPC knows Player is taking a risk', '例：\n• プレイヤーとNPCは信頼関係を築いた\n• NPCはプレイヤーがリスクを負っていることを知っている'],
        ...
        '角色面板分頁': ['Character panel tabs', 'キャラクターパネルのタブ'],
