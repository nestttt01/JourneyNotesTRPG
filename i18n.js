...
'角色面板可以查看與修改玩家狀態、NPC、道具、Flags、任務、劇情摘要及角色關係；AI 每回合會參考這些重點資料來延續故事。冒險日誌記錄完整事件，日記則用來收起想保留的對話片段；選「收藏這段」後，再點另一則當結尾，就會把連續對話收進日記。': ['The character panel lets you view and edit player status, NPCs, items, Flags, tasks, story summaries, and relationships; the AI uses these notes each turn to continue the story. Adventure Log keeps full events, while Diary saves dialogue scenes you want to keep. After choosing "Add to Diary", click another message as the end to save the continuous dialogue into Diary.', 'キャラクターパネルでは、プレイヤー状態、NPC、アイテム、Flags、タスク、物語の要約、関係性を確認・編集できます。AIは毎ターンこれらを参照して物語を続けます。冒険日誌は出来事全体を記録し、日記は残しておきたい会話場面を保存します。「日記に追加」を選んだあと、終点にしたい発言をもう一つ選ぶと、連続した会話が日記に保存されます。'],
