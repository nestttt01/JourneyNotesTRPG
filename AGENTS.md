...
- 使用者直接要求某個 AI 修改，即視為把本次寫入權交給該 AI，不需要特定「交棒」措辭；單純轉貼審查意見、詢問結論或要求唯讀檢查，不視為寫入授權。
...
`AGENTS.md`、`PROJECT_MAP.md`、`TECH_DEBT.md`、`CLAUDE.md` 自 2026-07-11 起納入 Git 追蹤：修改這四檔時比照程式檔的安全寫檔流程，以 `git show HEAD:<檔>` 作寫前基準與跨通道比對，不再強制工作區外交易備份（做了更好）。
...
- AI／工具環境可以使用唯讀 Git 指令：`git status`、`git diff`、`git log`、`git show`、`git ls-files`、`git check-ignore`。
- AI／工具環境不得執行任何 Git 寫入：`add`、`commit`、`push`、`reset`、`checkout`、`switch`、`branch`、`merge`、`rebase`、`stash`、`clean`，也不得直接改寫 `.git`。
- Git 寫入一律由使用者在自己的 PowerShell 或 `一鍵上傳.bat` 執行。
- 使用者 commit 後應執行 `git show --stat HEAD`，確認不是異常大量刪除後再視為完成。
...
- 不要為了解決單一畫面跑版使用大量 `!important` 壓過既有規則。
