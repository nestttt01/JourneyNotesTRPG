(function () {
    ...
    const M = {
'SAN 質疑': ['SAN doubt', 'SANの疑念'],
'HP 警訊': ['HP warning', 'HP警告'],
'你確定這是正確的選擇嗎？': ['Are you sure this is the right choice?', '本当にこれが正しい選択ですか？'],
'日誌已經寫過「{text}」。你現在要假裝沒發生？': ['The log already says “{text}.” Are you going to pretend it did not happen?', 'ログにはすでに「{text}」と書かれています。なかったことにするつもりですか？'],
'如果「{text}」是真的，這個選擇還安全嗎？': ['If “{text}” is true, is this choice still safe?', '「{text}」が本当なら、この選択はまだ安全ですか？'],
'你剛才不是已經知道「{text}」了嗎？': ['Did you not already learn “{text}”?', 'さっき「{text}」を知ったばかりではありませんか？'],
'摘要裡還留著「{text}」。你要違背它？': ['The summary still keeps “{text}.” Are you going against it?', '要約にはまだ「{text}」が残っています。それに逆らうつもりですか？'],
'主線不是正在指向「{text}」嗎？': ['Is the main thread not already pointing toward “{text}”?', '本筋は「{text}」へ向かっているのではありませんか？'],
'你確定這不是在逃避「{text}」？': ['Are you sure this is not running away from “{text}”?', 'これは「{text}」から逃げているだけではありませんか？'],
'任務還沒結束：「{text}」。現在要偏離嗎？': ['The task is not finished: “{text}.” Are you drifting away now?', '任務はまだ終わっていません：「{text}」。今それるつもりですか？'],
'你是不是忘了「{text}」？': ['Did you forget “{text}”?', '「{text}」を忘れたのですか？'],
'這個選項會讓「{text}」更難完成吧？': ['This choice would make “{text}” harder to finish, would it not?', 'この選択は「{text}」をさらに難しくするのではありませんか？'],
'關係記錄寫著「{text}」。你要拿它冒險？': ['The relationship record says “{text}.” Are you risking it?', '関係記録には「{text}」とあります。それを危険にさらすのですか？'],
'如果「{text}」還算數，你真的要這樣選？': ['If “{text}” still counts, are you really choosing this?', '「{text}」がまだ有効なら、本当にそう選ぶのですか？'],
'這會不會傷到「{text}」？': ['Could this hurt “{text}”?', 'これは「{text}」を傷つけませんか？'],
'Flags 還亮著：「{text}」。你確定？': ['Flags are still lit: “{text}.” Are you sure?', 'Flagsはまだ点灯しています：「{text}」。本当に？'],
'系統沒有忘記「{text}」。你忘了嗎？': ['The system has not forgotten “{text}.” Did you?', 'システムは「{text}」を忘れていません。あなたは忘れましたか？'],
'在「{text}」底下，這個選項可靠嗎？': ['Under “{text},” is this option reliable?', '「{text}」のもとで、この選択肢は信頼できますか？'],
'現在還在「{text}」。你真的能離開這條線？': ['You are still in “{text}.” Can you really leave this thread?', '今も「{text}」の中です。本当にこの流れから離れられますか？'],
'「{text}」不是偶然被選中的場景。': ['“{text}” was not chosen by accident.', '「{text}」は偶然選ばれた場面ではありません。'],
'這個選擇和「{text}」對得上嗎？': ['Does this choice match “{text}”?', 'この選択は「{text}」と噛み合っていますか？'],
'「{text}」會怎麼看這個選擇？': ['How would “{text}” see this choice?', '「{text}」はこの選択をどう見るでしょうか？'],
'你要讓「{text}」承受後果嗎？': ['Are you making “{text}” carry the consequences?', '「{text}」にその結果を背負わせるのですか？'],
'如果「{text}」知道了呢？': ['What if “{text}” finds out?', 'もし「{text}」が知ったら？'],'\u8ddf {npc} \u7684\u8ddd\u96e2\u62c9\u8fd1\u4e86\u4e00\u9ede': ['You feel a little closer to {npc}.', '{npc}\u3068\u306e\u8ddd\u96e2\u304c\u5c11\u3057\u7e2e\u307e\u3063\u305f'],
...
'新增低 HP／低 SAN 干擾：狀態進入危機區間時，畫面會出現暗角、顆粒、掃描線、短暫亂碼與狀態訊號，且配色會跟著目前介面主題變化。': ['Added low-HP / low-SAN interference: when survival status enters the crisis range, the screen shows vignette, grain, scan lines, brief text corruption, and status signals, with colors following the current UI theme.', '低HP／低SAN干渉を追加：生存状態が危機域に入ると、画面に暗い縁取り、粒子、走査線、一時的な文字化け、状態シグナルが現れ、配色は現在のUIテーマに連動します。'], '低 SAN 選項會變得不可靠：點選時可能變色並改成另一個可見選項，系統會直接送出改變後的內容。': ['Low-SAN options become unreliable: clicking one may change its color, turn into another visible option, and immediately submit the changed text.', 'SANが低いと選択肢が不安定になります：クリックした瞬間に色が変わり、別の表示中の選択肢に書き換わって、その変更後の内容がそのまま送信されることがあります。'], 'HP 與 SAN 代表角色的生存狀態。HP 或 SAN 低於 20 時會進入危機區間，部分判定會受影響；畫面也會出現暗角、底片顆粒、掃描線、短暫文字亂碼與狀態訊號，但配色會跟著目前介面主題走。': ['HP and SAN represent your character’s survival state. When HP or SAN drops below 20, the character enters a crisis range and some checks are affected; the screen also gains vignette, film grain, scan lines, brief text corruption, and status signals, all colored by the current UI theme.', 'HPとSANはキャラクターの生存状態を表します。HPまたはSANが20未満になると危機域に入り、一部の判定に影響します。画面にも暗い縁取り、フィルム粒子、走査線、一時的な文字化け、状態シグナルが現れますが、配色は現在のUIテーマに合わせられます。'], 'SAN 很低時，AI 提供的選項可能變得不可靠。被污染的選項在點下去的瞬間會變色、文字改成另一個可見選項，並立刻送出改變後的內容；如果沒有觸發污染，點選仍會像平常一樣先填入輸入框。': ['When SAN is very low, AI options can become unreliable. A corrupted option changes color the moment you click it, rewrites itself into another visible option, and immediately submits the changed text; if corruption does not trigger, clicking still fills the input field as usual.', 'SANが非常に低いと、AIの選択肢が不安定になることがあります。汚染された選択肢はクリックした瞬間に色が変わり、別の表示中の選択肢へ書き換わり、その変更後の内容がすぐ送信されます。汚染が発生しなかった場合は、通常どおり入力欄に入るだけです。'], '新增對話右鍵／手機長按選單：可編輯已送出的文字、複製對話，或把片段收藏到日記。': ['Added message right-click / mobile long-press menu: edit sent text, copy dialogue, or add a scene to Diary.', '発言の右クリック／スマホ長押しメニューを追加：送信済みテキストの編集、会話のコピー、場面を日記に保存できます。'],
