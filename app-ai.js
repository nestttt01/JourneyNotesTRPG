// === [app.js 拆分] app-ai.js：原 app.js 第 6906–7661 行｜AI 請求/JSON 解析修復/語言修復/記憶整理/prompt 組裝/callAI_JSON(危險區)｜需依 index.html 既有順序與其他 app-*.js 一同載入，勿單獨重排。 ===
        ...
        function getAIResponseText(data) {
            ...
            const gCand = data.candidates?.[0];
            if (data.promptFeedback?.blockReason) {
                const e = new Error(`你這次輸入的內容被 Gemini 直接擋下了——這是它最底層的限制，連「成人模式」也解不開。可以換個說法，或改用 OpenRouter 的模型。`);
                e.userFriendly = true; throw e;
            }
            if (gCand?.finishReason === 'SAFETY') {
                const cats = (gCand.safetyRatings || []).filter(r => r.blocked).map(r => valueToText(r.category).replace('HARM_CATEGORY_', '')).join('、') || '未指明';
                const e = new Error(`Gemini 把這次的回覆擋掉了（超出它願意輸出的尺度）。就算開了「成人模式」，Gemini 有些內容還是不會寫。想要更開放，建議改用 OpenRouter 的無審查模型。`);
                e.userFriendly = true; throw e;
            }
            return gCand?.content?.parts?.[0]?.text || "";
        }
            ...
            for (let attempt = 1; attempt <= attemptLimit; attempt += 1) {
                ...
                try {
                    ...
                    if (!response.ok || data.error) {
                        ...
                        if (apiProvider === 'google' && response.status === 400 && /block_none|safety|sexually|harm_category/i.test(errMsg)) {
                            const e = new Error(`這把 Gemini 金鑰沒辦法放寬內容限制（你的 Google 帳號不允許），所以「成人模式」對 Gemini 沒有效果。想跑成人向內容，建議改用 OpenRouter 的無審查模型。（技術診斷：${errMsg}）`);
                            e.userFriendly = true; throw e;
                        }
...
${JSON.stringify(visiblePayload)}`;
 ...
 const finalLog = restoreProtectedAdventureLogEntries(currentAdventureLog, importantLog);
