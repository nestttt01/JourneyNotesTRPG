...
heart: '--heart-base'
        ...
        const defaultPreset = {
            ...
            scenarios: [
                {
                    name: '序章：出門前的小演練（新手教學）',
                    lore: '現代都市，小島秀芙的公寓清晨。今天三人約好去咖啡廳的「異寵交流日」，宮崎三高提早抵達，順便陪秀芙做出門準備。\n【教學情境：DM 引導守則】本情境是新手教學，允許 DM 與 NPC 後設地提及遊戲介面（輸入框、擲骰按鈕、右側角色面板、道具的「使用」按鈕）。請按下列順序一步一步教，每一步等玩家實際做出行動後再進下一步，不要一次全部講完：\n1. 開場：宮崎三高抵達，請玩家在輸入框輸入任何想說或想做的事試試看。\n2. 擲骰：安排一個輕鬆的小狀況（例如大野智爬上高處的櫃子），鼓勵玩家按「⚄ 擲骰」解決；無論成敗都溫柔收尾、不扣血。\n3. 道具：宮崎三高遞上親手做的早餐三明治——用 items_add 給「三高的早餐三明治」並在 item_effects 標 san 10，提醒玩家打開右側「角色面板」在道具區按「使用」吃掉它。\n4. 好感：玩家吃了三明治或做出任何友善互動後，回傳宮崎三高好感 +5。\n5. 收尾：以上都完成後宣布準備出門，回傳 objective_complete 讓本場目標達成。\n節奏保持輕鬆日常，不要出現任何危險、扣血或負面事件。',
                    npcRoles: '宮崎三高是本場的引導者：話少但溫柔，每次只用最精簡的話告訴玩家「現在可以試什麼」。SY 本場不在場，偶爾透過手機訊息冒出來損宮崎三高兩句即可。',
                    playerRole: '玩家剛起床準備出門，跟著宮崎三高的引導，依序熟悉行動輸入、擲骰判定、道具使用與好感互動。',
                    transitionRule: '教學完成、兩人出門後，切換至「稀有品種的咖啡廳巡禮」情境。',
                    objective: '完成出門前的小演練（試著輸入行動、擲一次骰、使用一件道具）'
                },
                {
                    name: '稀有品種的咖啡廳巡禮',
