...
"想安排環境、鏡頭或讓 NPC 行動，用 ［切換旁白］／［旁白］；切換回玩家用 ［切換玩家］／［玩家］；想直接改場景、時間、人物，用「神」按鈕。這些都不會被當成玩家發言。": ["To arrange scenery, camera or NPC actions use ［切換旁白］/［旁白］; to switch back use ［切換玩家］/［玩家］; to directly change scene, time or characters use the \"God\" button. None of these count as the player speaking.", "環境・カメラ・NPC の行動を配置するには ［切換旁白］/［旁白］、プレイヤーに戻すには ［切換玩家］/［玩家］、場面・時間・人物を直接変えるには「神」ボタン。いずれもプレイヤーの発言にはなりません。"],
...
"覺得干擾太多，可在「顯示與輸出設定 → 低狀態干擾」調成減弱或關閉。": ["If it's too much, set \"Display & Output → Low-state effects\" to Reduced or Off.", "演出が多すぎる場合は「表示と出力設定 → 低ステート演出」を控えめかオフにできます。"],
...
"「喘息」鈕可主動擲檢定回一點 HP／SAN，用來脫離死亡螺旋（回復上限為 HP／SAN 總量的 50%，不能連按）。": ["The \"Catch Breath\" button rolls to restore some HP/SAN to escape a death spiral (capped at 50% of max, no consecutive use).", "「ひと息」ボタンは判定で HP／SAN を少し回復し、死のスパイラルから抜けます（上限は最大値の50%、連続使用不可）。"],
...
"故事變長後 AI 可能變鈍，這是模型限制。可以換標有 ▪ 的模型、多用「整理摘要／整理紀錄」壓縮記憶，或開「神」模式直接說明目前場景與重點。": ["As the story grows the AI may dull, a model limit. Switch to a ▪-marked model, use \"Organize summary / log\" often to compress memory, or use \"God\" mode to state the current scene and key points.", "物語が長くなると AI が鈍ることがあります（モデルの限界）。▪ 付きモデルに変える、「要約整理／記録整理」で記憶を圧縮する、または「神」モードで現在の場面と要点を伝えます。"],
"每份遊戲紀錄綁定一份配置；另存新檔會複製紀錄並建立專屬配置。": ["Each save binds to one preset; \"save as new\" copies the save and creates its own preset.", "各セーブは1つのプリセットに紐づき、「新規保存」はセーブを複製して専用プリセットを作ります。"],
...
"成人模式在狀態面板開關（預設關），開啟後 AI 會在角色皆成年且合意的前提下更自然承接親密場景，實際尺度仍取決於所選模型。": ["Mature mode toggles in the status panel (off by default); when on, the AI handles intimate scenes more naturally on the premise that all characters are consenting adults, though the actual explicitness depends on the model.", "成人モードはステータスパネルで切替（既定オフ）。オンにすると全員が成人かつ合意の前提で AI が親密シーンをより自然に扱いますが、描写度合いはモデル次第です。"],
...
'細節：AI 選項逐顆入場、主要按鈕金屬光澤、判定中像素讀取動畫、低 SAN 時輸入框提示偶爾亂碼、存檔成功右下角落印、NPC 死亡頭像轉灰、對話泡泡尖角方向修正、骰子與情境圖示改用像素符號。': ['Details: options enter one by one, primary buttons gained a metallic shine, a pixel loader during checks, the input placeholder occasionally glitches at low SAN, a save pip in the corner, avatars grey out on NPC death, bubble tails now point the right way, and dice/scene icons use pixel glyphs.', '細部：選択肢の順次登場、主要ボタンのメタリックシャイン、判定中のピクセルローダー、低 SAN 時の入力欄ノイズ、セーブ成功の隅ランプ、NPC 死亡時のアバターグレー化、吹き出しの尻尾の向き修正、ダイス／シナリオ記号のピクセル化。'],
...
'另存新檔': ['Save New File', '新規データとして保存'],
        ...
        '閱讀與整理': ['Read & organize', '閲覧・整理'], '摘要內容': ['Summary', 'サマリー'],
        '重點／任務／關係': ['Highlights / Tasks / Relationships', '重要事項／タスク／関係'],
        '完整紀錄': ['Full record', '完全記録'], '完整冒險日誌': ['Full Adventure Log', '完全な冒険ログ'],
...
'標準模式適合日常與長篇劇情；困難模式會提高檢定 DC，NPC 每次死亡只有一次復活檢定；極限模式會進一步提高 DC，角色歸零會觸發「最後掙扎」，生死檢定 D20 ≥ 8 才能存活，NPC 死亡後無法復活。': ['Standard mode suits slice-of-life and long stories. Hard mode raises check DCs, and each NPC death allows only one revival check. Nightmare mode raises DCs further; reaching zero triggers a last stand, and you survive on a D20 roll of 8 or higher; NPC death cannot be reversed.', '標準モードは日常や長編向きです。ハードモードでは判定DCが上がり、NPC死亡ごとに復活判定は1回だけです。ナイトメアモードではさらにDCが上がり、キャラクターがゼロになると「最後の抵抗」が発生し、生死判定でD20が8以上なら生存します。NPCは死亡後に復活できません。'],
